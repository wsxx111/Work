...
using WK.Repository.SystemSecurity;

namespace WK.Application.SystemSecurity
{
    public class LogApp
    {
        ...
        public List<LogEntity> GetList(Pagination pagination, string queryJson)
        {
            ...
            if (!string.IsNullOrWhiteSpace(queryParam["keyword"].ToString()))
            {
                string keyword = queryParam["keyword"].ToString();
                expression = expression.And(t => t.F_Account.Contains(keyword));
            }
            if (!string.IsNullOrWhiteSpace(queryParam["timeType"].ToString()))
            {
        ...
        public void WriteDbLog(bool result, string resultLog)
        {
            ...
            logEntity.F_IPAddressName = "";// WebHelper.GetLocation(logEntity.F_IPAddress);
